...
int main()
{
	...
	printf("Nhap n la so dong va so cot cua ma tran : n="); scanf("%d",&n);
	nhap(a,n);
	xuat(a,n);
	max(a,n);
	min(a,n);
	...
	chan(a,n);
	printf("\n\n\n\n\n\n\n");
	printf("\n\n                                XAY DUNG MA TRAN A va B VA TINH TONG TICH CUA A va B                   \n\n\n");
	printf("\n\nNhap m la so dong cho A va B : m = "); scanf("%d",&M);
	printf("\n\nNhap n la so cot cho A va B : n = ");  scanf("%d",&N);
	avb(A,B,C,M,N);
...
void snt(int *a, int n)
{
	 ...
	 printf("\n\nCo %d phan tu co gia tri la so nguyen to trong ma tran va tong cua chung bang %d",dem,tong);
}
...
void timx(int *a, int n)
{
	 ...
	 for (int i=0;i<n;i++)
	 {
	 	 for (int j=0;j<n;j++)
	 	 {
	 	 	 ...
	 	 	 {
	 	 	 	OK++;
	 	 	 	printf("\n\nx xuat hien tai dong thu %d, cot thu %d (vi tri U[%d][%d]) trong ma tran",i+1,j+1,i,j);
	 	 	 }
	 	 }
	 }
	 if (OK==0) printf("\n\nx khong xuat hien trong ma tran !");
}
...
void tongdcc(int *a, int n)
{
	 ...
	 printf("\n\nTong cac phan tu nam tren duong cheo chinh cua ma tran = %d",x);
}
...
void chan(int *a,int n)
{
    ...
    if (dem==0) printf("\n\nKhong co phan tu nao la so chan nam trong tam giac tren cua duong cheo chinh cua ma tran !");
    else        printf("\n\nCo %d phan tu la so chan nam trong tam giac tren cua duong cheo chinh cua ma tran ma khong ke duong cheo chinh !",dem);
}
